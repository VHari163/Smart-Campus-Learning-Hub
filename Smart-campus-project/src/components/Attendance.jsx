...
export default function Attendance({ attendance, setAttendance, onAddActivity }) {
  ...
  const parsed = useMemo(() => {
    const a = Number(String(attended ?? '').trim())
    const t = Number(String(total ?? '').trim())
    const ok = Number.isFinite(a) && Number.isFinite(t) && t > 0 && a >= 0 && a <= t
  ...
  const status = useMemo(() => {
    ...
    if (parsed.pct >= 75) return { label: 'Good Attendance (Green)', colorClass: 'ok' }
    return { label: 'Needs Improvement (Red)', colorClass: 'bad' }
  }, [parsed])

  const exDisplay = useMemo(() => {
    const typed = exText?.trim()
    if (typed) return typed
    const fromStorage = attendance?.exText?.trim()
    return fromStorage || ''
        ...
        <div className="glass card">
          ...
          <div className="resultBig">
            <div className={`resultStatus ${status.colorClass}`}>
              <div className="resultPct">{parsed.ok ? `${parsed.pct}%` : '—'}</div>
